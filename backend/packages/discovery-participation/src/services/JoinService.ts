...
export class JoinService {
  ...
  async joinActivity(studentAccountId: string, campusId: string, activityId: string): Promise<Participation> {
    return executeTransaction(this.dataSource, async (manager) => {
      ...
      if (existing) {
        throw AppError.conflict("You have already joined or requested to join this activity", "Participation");
      }
      ...
      if (activity.participationMode === ParticipationMode.Open) {
        ...
        if (activity.maxRequests !== null && activity.currentRequestCount >= activity.maxRequests) {
          throw AppError.conflict("Activity has reached the maximum number of pending requests", "Activity");
        }
      ...
      // 7. Emit event to Notification System
      await this.eventDispatcher.dispatch(eventName, { participationId: savedParticipation.participationId, activityId: activity.activityId, studentAccountId, hostAccountId: activity.hostAccountId });
