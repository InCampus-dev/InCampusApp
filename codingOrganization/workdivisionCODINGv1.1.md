...
| AP12 | AP | Implement AccountModerationCommandHandler: receive RequestAccountModerationAction from SM, update DS-AP-001.PlatformAccessStatus | UCR - A&P v1.2.md (Events Consumed, Internal Interfaces) | S | Jacopo |
| **CA** | | | | |
...
| HL07 | H&L | Implement ActivityModerationCommandHandler: receive RequestActivityModerationAction from SM, execute native hard-delete workflow on DS-HL-001 and DS-HL-002 | UCR - H&L v1.4.md (Events Consumed) | S | Matteo |
| **D&P** | | | | |
...
| **NSF09** | NSF | Notification list endpoint: GET /notifications (paginated, recipient-scoped, read-only; no isRead/readAt field per architecture) – *Conditional: added only if M06 remains a notification list screen (see UV01)* | FR-0704, UCR - N&S v1.2.md open point 3 | M | Jacopo |
| **SM** | | | | |
...
| M06 | Mobile | Basic notification display (in‑app list) ⚠️ UNVERIFIED | DUC‑NSF‑06 | S | Jacopo |

> ⚠️ **UNVERIFIED – M06:** No `GET /notifications` list endpoint is defined in any UCR. The notification list UX is marked “Unresolved” (UCR - N&S v1.2.md open point 3). The task NSF09 (conditional) provides a backend endpoint if the team decides to keep the in-app list. Otherwise, rescope M06 to a single-notification context navigation (see MOD03 option B in the revision). **Action required before Day 1.**
...
| Developer | Tasks | Dependencies |
|-----------|-------|--------------|
| **Francesco** | SM03 (Report Submission), SM04 (Review Report + moderation dispatch to AP/H&L), **NSF04 (Cancellation fan‑out)**, **NSF05 (LeaveEvent)**. Write integration tests. | SM03 depends on AP and H&L entities for target validation; SM04 uses AP/H&L internal commands. NSF04 needs ActivityCancelled emitted from HL04 (done today by Matteo); NSF05 needs JoinedParticipantLeft from DP05. Mitigation: NSF04 and NSF05 handlers must be testable with synthetic events from Day 3 onward (see NSF02/NSF03 pattern). If HL04 is not merged before NSF04 integration, NSF04 integration tests use a synthetic ActivityCancelled event stub and are validated end-to-end on Day 5. |
| **Jacopo**   | NSF06 (Reminder), NSF07 (Open Notification). AP11 (unit tests). Mobile M02 (sign‑up flow). | AP11 and NSF07 are independent; NSF06 relies on time trigger stub. |
...
| **Jacopo**   | NSF08 (tests), M03 (campus/profile screens), M06 (notification list, if confirmed). Final integration tests for AP/NSF flows. | Mobile screens need working backend endpoints. |
...
| `src/controllers/NotificationListController.ts` | GET /notifications endpoint — returns recipient-scoped notification records (no read/unread state). *Conditional: added only if M06 remains a notification list screen.* | Jacopo |
...
| `src/screens/NotificationListScreen.tsx` | Notification inbox (⚠️ scope pending team decision per UV01) | Jacopo |
...
> **Note:** All business logic tasks are traceable to the project documentation (UCRs, Sequence Diagrams, CRUD Matrix, Entity Catalog, and Architectural Choice document). Implementation technology choices — Node.js, TypeScript, TypeORM, PostgreSQL, React Native — are team implementation decisions not specified in any project document and should be confirmed by the team before Day 1.
